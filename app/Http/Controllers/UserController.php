...
class UserController extends Controller
{
    ...
    public function result(Request $request)
    {
      ...
      $mainSQL = DB::table('employee')->select('employee.number','name','age')->JoinSub($subSQL, 'bus', 'employee.number', 'bus.number')->setBindings([':experience_code'=>1, ':experience_class_code'=>7, ':level'=>1]);

      //$subSQL = DB::table('business_experience')->where('experience_code', '=', ':experience_code')->where('experience_class_code', '=', ':experience_class_code')->where('level', '>', ':level')->toSQL();
      //$mainSQL = $mainSQL->JoinSub($subSQL, 'bus', 'employee.number', 'bus.number')->setBindings([':experience_code'=>1, ':experience_class_code'=>7, ':level'=>2]);
